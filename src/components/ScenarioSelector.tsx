          ...
          {row2.map((scenario, i) => {
            ...
            return (
              <motion.button
                key={scenario.id}
                onClick={() => onSelect(scenario)}
                className="w-full text-left rounded-lg border border-[#222222] bg-[#111111] hover:border-[#383838] hover:bg-[#161616] p-5 md:p-6 cursor-pointer transition-colors duration-[120ms] ease-out flex flex-col"
                style={{ flexBasis: "calc((100% - 32px) / 3)", flexGrow: 0, flexShrink: 0 }}
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
